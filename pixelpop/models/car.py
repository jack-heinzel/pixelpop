...
class ICAR_length_scales(Distribution):
    ...
    log_sigmas : array-like, shape (dimension,) or scalar
        Logarithm of the standard deviation(s) of the ICAR prior. 
        Can provide a separate log_sigma for each dimension; otherwise,
    ...
    def __init__(
        ...
        *,
        ...
        if jnp.ndim(log_sigmas) == 0:
            ...
            assert jnp.shape(log_sigmas)[-1] == self.dimension
    ...
    @property
    def support(self):
        # tensor with dimension = self.dimension
    ...
    def log_prob(self, phi):
        ...
        # print(ar.shape)
        logdet = jnp.sum(jnp.log(ar.at[(0,)*self.dimension].set(jnp.sum(precs))))
        logquad = 0.
